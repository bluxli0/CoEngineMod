#include "MathUtils.hpp"

namespace CoEngine {

    const int MathUtils::s_permutation[512] = {
        151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,
        190, 6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,88,237,149,56,87,174,
        20,125,136,171,168, 68,175,74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,
        230,220,105,92,41,55,46,245,40,244,102,143,54, 65,25,63,161, 1,216,80,73,209,76,132,187,208, 89,
        18,169,200,196,135,130,116,188,159,86,164,100,109,198,173,186, 3,64,52,217,226,250,124,123,5,202,
        38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,223,183,170,213,119,248,152,
        2,44,154,163, 70,221,153,101,155,167, 43,172,9,129,22,39,253, 19,98,108,110,79,113,224,232,178,185,
        112,104,218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241, 81,51,145,235,249,14,239,107,
        49,192,214, 31,181,199,106,157,184, 84,204,176,115,121,50,45,127, 4,150,254,138,236,205,93,222,114,
        67,29,24,72,243,141,128,195,78,66,215,61,156,180,
        // Wrapped block
        151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,
        190, 6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,88,237,149,56,87,174,
        20,125,136,171,168, 68,175,74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,
        230,220,105,92,41,55,46,245,40,244,102,143,54, 65,25,63,161, 1,216,80,73,209,76,132,187,208, 89,
        18,169,200,196,135,130,116,188,159,86,164,100,109,198,173,186, 3,64,52,217,226,250,124,123,5,202,
        38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,223,183,170,213,119,248,152,
        2,44,154,163, 70,221,153,101,155,167, 43,172,9,129,22,39,253, 19,98,108,110,79,113,224,232,178,185,
        112,104,218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241, 81,51,145,235,249,14,239,107,
        49,192,214, 31,181,199,106,157,184, 84,204,176,115,121,50,45,127, 4,150,254,138,236,205,93,222,114,
        67,29,24,72,243,141,128,195,78,66,215,61,156,180
    };

    float MathUtils::fade(float t) {
        return t * t * t * (t * (t * 6.0f - 15.0f) + 10.0f);
    }

    float MathUtils::lerp(float t, float a, float b) {
        return a + t * (b - a);
    }

    float MathUtils::grad(int hash, float x, float y) {
        int h = hash & 7;
        float u = h < 4 ? x : y;
        float v = h < 4 ? y : x;
        return ((h & 1) ? -u : u) + ((h & 2) ? -2.0f * v : 2.0f * v);
    }

    float MathUtils::getSeededRandom(uint32_t seed, int x, int y) {
        uint32_t hash = seed;
        hash ^= static_cast<uint32_t>(x);
        hash = (hash ^ 61) ^ (hash >> 16);
        hash += hash << 3;
        hash ^= static_cast<uint32_t>(y);
        hash ^= hash >> 4;
        hash *= 0x27d4eb2d;
        hash ^= hash >> 15;
        return static_cast<float>(hash) / static_cast<float>(UINT32_MAX);
    }

    float MathUtils::getPerlinNoise2D(float x, float y, uint32_t seed) {
        x += static_cast<float>((seed & 0xFFFF));
        y += static_cast<float>((seed >> 16) & 0xFFFF);

        int X = static_cast<int>(std::floor(x)) & 255;
        int Y = static_cast<int>(std::floor(y)) & 255;

        x -= std::floor(x);
        y -= std::floor(y);

        float u = fade(x);
        float v = fade(y);

        int a = s_permutation[X] + Y;
        int b = s_permutation[X + 1] + Y;

        return lerp(v, lerp(u, grad(s_permutation[a], x, y),
                               grad(s_permutation[b], x - 1, y)),
                       lerp(u, grad(s_permutation[a + 1], x, y - 1),
                               grad(s_permutation[b + 1], x - 1, y - 1)));
    }

    cocos2d::CCPoint MathUtils::gridToWorld(int gridX, int gridY) {
        return cocos2d::CCPoint{
            (static_cast<float>(gridX) * GRID_UNIT) + 15.0f,
            (static_cast<float>(gridY) * GRID_UNIT) + 15.0f
        };
    }

    cocos2d::CCPoint MathUtils::worldToGrid(const cocos2d::CCPoint& worldPos) {
        return cocos2d::CCPoint{
            std::floor(worldPos.x / GRID_UNIT),
            std::floor(worldPos.y / GRID_UNIT)
        };
    }

    float MathUtils::easeInOutQuad(float t) {
        return t < 0.5f ? 2.0f * t * t : 1.0f - std::pow(-2.0f * t + 2.0f, 2.0f) / 2.0f;
    }

    float MathUtils::easeOutCubic(float t) {
        return 1.0f - std::pow(1.0f - t, 3.0f);
    }
}
